...
Generate a secure admin password and save it:
...
**Analytics secrets (optional)** — required for the stats panel on `/dashboard`:

```bash
npx wrangler secret put CF_ZONE_ID
# paste your Zone ID — right sidebar of dash.cloudflare.com → your domain

npx wrangler secret put CF_API_TOKEN
# paste an API token with Zone Analytics Read permission
# create one at: https://dash.cloudflare.com/profile/api-tokens
```

With `CF_ZONE_ID` set, the stats panel shows **real hit counts including CDN-cached redirects** (last 90 days). Without it, only Worker invocations are counted.

If you skip analytics secrets entirely, `/dashboard` still shows the full links list — only the per-link stats panel will show "Analytics not configured."
...
- [ ] `CF_ZONE_ID` secret set (enables real hit counts including CDN cache on /dashboard)
- [ ] `CF_API_TOKEN` secret set (needs Zone Analytics Read permission)
